...
const AdminDashboard = () => {
  ...
  const getCategoryBadge = (category) => {
    switch(category) {
      ...
      case "warm":
        return <Badge variant="warning" className="bg-yellow-500 hover:bg-yellow-600">Warm Lead</Badge>;
      case "cold":
        return <Badge variant="secondary" className="bg-blue-500 text-white hover:bg-blue-600">Cold Lead</Badge>;
      default:
  ...
  return (
    <Card className="max-w-6xl mx-auto my-8 shadow-lg">
      <CardHeader className="bg-slate-50 dark:bg-slate-800">
        <CardTitle className="text-2xl font-bold">Admin Dashboard - Leads</CardTitle>
      </CardHeader>
      <CardContent className="p-6">
        ...
        <Dialog open={!!selectedLead} onOpenChange={(open) => !open && setSelectedLead(null)}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold">Score Breakdown</DialogTitle>
            </DialogHeader>
            {selectedLead && (
                  ...
                  {selectedLead.actions.length > 0 ? (
                    <ul className="space-y-2">
                      {selectedLead.actions.map((action, index) => (
                        <li key={index} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-800 rounded-md">
                          <span>{action.action}</span>
                          <Badge variant="outline" className="font-bold">+{action.score || 0} points</Badge>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-muted-foreground italic">No recorded actions</p>
                  )}
                </div>
              </div>
            )}
